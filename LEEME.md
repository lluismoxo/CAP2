...
1. El usuario rellena el formulario de la sección "Contacto".
2. El navegador hace un `POST /api/contact` con los datos en JSON.
3. El backend valida (nombre, email, anti-spam honeypot, rate-limit por IP).
4. Guarda el contacto en `data/contacts.json` con escritura atómica.
5. Muestra al usuario un mensaje de éxito o de error.
